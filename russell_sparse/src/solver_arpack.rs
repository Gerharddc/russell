...
pub type ArpackComplex = arpack_ffi::__BindgenComplex<f64>;
...
impl Default for ArpackConfig {
    fn default() -> Self {
        Self {
            ...
            tolerance: 0.0,
            shift: ArpackComplex { re: 0.0, im: 0.0 },
        }
    }
}
...
pub struct ArpackResults {
    ...
    pub eigenvalues: Vec<ArpackComplex>,
    /// The computed eigenvectors (stored as columns in row-major order)
    pub eigenvectors: Vec<ArpackComplex>,
...
where
    F: FnMut(&mut S, &[ArpackComplex], &mut [ArpackComplex]) + 'a,
    G: FnMut(&mut S, &[ArpackComplex], &mut [ArpackComplex], ArpackComplex) + 'a,
{
    ...
    G: FnMut(&mut S, &[ArpackComplex], &mut [ArpackComplex], ArpackComplex) + 'a,
{
    ...
    pub fn new(dimension: usize, config: ArpackConfig, matrix_vector_product: F, linear_solve: G, state: S) -> Self {
    ...
    pub fn solve(&mut self) -> Result<ArpackResults, StrError> {
        ...
        let which = self.config.which_eigenvalues.arpack_str().as_ptr();
        ...
        let mut resid = vec![ArpackComplex { re: 0.0, im: 0.0 }; self.dimension];
        ...
        let mut v = vec![ArpackComplex { re: 0.0, im: 0.0 }; self.dimension * self.config.num_arnoldi_vectors];
        ...
        let mut workd = vec![ArpackComplex { re: 0.0, im: 0.0 }; 3 * self.dimension];

        // Working array for ARPACK internal computations
        //      Must be at least 3*ncv^2 + 5*ncv in size
        let lworkl = 3 * ncv * ncv + 5 * ncv;
        let mut workl = vec![ArpackComplex { re: 0.0, im: 0.0 }; lworkl as usize];
        ...
        loop {
            ...
            match ido {
                ...
                -1 | 1 => {
                    ...
                    let in_stop = in_start + self.dimension;
                    let out_stop = out_start + self.dimension;
                    ...
                    (self.linear_solve)(&mut self.state, input, output, sigma);
                }
        ...
        let mut d = vec![ArpackComplex { re: 0.0, im: 0.0 }; (nev + 1) as usize];

        // Work array for zneupd
        //      Must be 2*ncv in size
        let mut workev = vec![ArpackComplex { re: 0.0, im: 0.0 }; 2 * ncv as usize];
